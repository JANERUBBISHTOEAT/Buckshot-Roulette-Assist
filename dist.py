...
def dist_given_a(aces_a):
    ...
    for aces_b, aces_c, aces_d in itertools.product(
        range(cards_per_player + 1), repeat=3
    ):
        ...
        ways_b = math.comb(total_aces, aces_b) * math.comb(total_non_aces, non_aces_b)
        ...
        ace_count_dist_given_a[max(aces_b, aces_c, aces_d)] = (
            ace_count_dist_given_a.get(max(aces_b, aces_c, aces_d), 0) + total_ways
        )
